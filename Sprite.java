...
public class Sprite {
	...
	public boolean collision(Sprite otherSprite) {
		if (isSmall) {
			return getSmallBounds().intersects(otherSprite.getBounds());
		}
	...
	public boolean collision(Item item) {
		if (isSmall) {
			return getSmallBounds().intersects(item.getBounds());
		}
		
		return getBounds().intersects(item.getBounds());
	}

	public boolean collision(Item2 item) {
		if (isSmall) {
			return getSmallBounds().intersects(item.getBounds());
		}
	...
	public boolean collision(Item3 item) {
		if (isSmall) {
			return getSmallBounds().intersects(item.getBounds());
		}
	...
	public boolean collision(Item4 item) {
		if (isSmall) {
			return getSmallBounds().intersects(item.getBounds());
		}
	...
	public Rectangle getSmallBounds(){
		if(x_direction < 0)
			return new Rectangle(x_coordinate + imageResource.getImageOffset()+80, y_coordinate, 
					(imageResource.getImage().getIconWidth() - imageResource.getImageOffset()/2)/2, 
					(imageResource.getImage().getIconHeight())/2);
		else
			return new Rectangle(x_coordinate + 2 * imageResource.getImageOffset(), y_coordinate, 
					(imageResource.getImage().getIconWidth() - imageResource.getImageOffset()/2)/2, 
					(imageResource.getImage().getIconHeight())/2);
	...
	public void draw(Graphics g, Component c) {
		...
		if(!isSmall)
			...
			if(x_direction < 0)
				g2.drawImage(imageResource.getImage().getImage(), x_coordinate + imageResource.getImage().getIconWidth() + imageResource.getImageOffset(), 
						y_coordinate, -imageResource.getImage().getIconWidth()/2, imageResource.getImage().getIconHeight()/2, null);
			else
				g2.drawImage(imageResource.getImage().getImage(), x_coordinate + imageResource.getImage().getIconWidth(), 
						y_coordinate, imageResource.getImage().getIconWidth()/2, imageResource.getImage().getIconHeight()/2, null);
	}
